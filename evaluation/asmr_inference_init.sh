...
#SBATCH --job-name=vas_infer_on_asmr
#SBATCH --partition=juno --qos=normal
######SBATCH --account=project_2000936
#SBATCH --chdir=/juno/u/jyau/SpecVQGAN
...
if [[ "$DATASET" == "VGGSound" ]]; then
    ...
    SPEC_DIR_PATH="/juno/u/jyau/regnet/data/features/ASMR/asmr_both_vids/melspec_10s_44100hz"
    RGB_FEATS_DIR_PATH="/juno/u/jyau/regnet/data/features/ASMR/asmr_both_vids/feature_rgb_bninception_dim1024_21.5fps/"
    FLOW_FEATS_DIR_PATH="/juno/u/jyau/regnet/data/features/ASMR/asmr_both_vids/feature_flow_bninception_dim1024_21.5fps/"
    SAMPLES_FOLDER="ASMR_validation"
    SPLITS="\"[validation, ]\""
    SAMPLER_BATCHSIZE=4
...
for TOP_K in "${TOP_K_OPTIONS[@]}"; do
    ...
    source /juno/u/jyau/anaconda3/etc/profile.d/conda.sh
    NODE_RANK=0
    for node in $HOSTLIST; do
        ssh -q $node "conda activate specvqgan && \
                    cd /juno/u/jyau/SpecVQGAN && \
